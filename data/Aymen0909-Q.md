...
| :-------------: |:-------------|:-------------:|:-------------:|
| 1      | Changing ownership missing check for `address(0)` in `setOwner` function | Low | 1 |
| 2      | Missing checks for `address(0)` when assigning values to `address` state variables | Low | 3 |
| 3      | Setters should check the input value and revert if it's the zero address or zero | Low | 3 |
| 4      | `require()`/`revert()` statements should have descriptive reason strings | NC | 26 |
| 5      | Use more recent solidity versions | NC |  |
| 6      | Named return variables not used anywhere in the functions | NC | 11 |
| 7      | `2**<N> - 1` should be re-written as `type(uint<N>).max`  | NC | 1 |
| 8      | `constant` should be defined rather than using magic numbers  | NC | 2 |

## Findings

### 1- Changing ownership missing check for `address(0)` in `setOwner` function :
...
### 2- Missing checks for `address(0)` when assigning values to `address` state variables  :
...
### 7- `2**<N> - 1` should be re-written as `type(uint<N>).max` :
